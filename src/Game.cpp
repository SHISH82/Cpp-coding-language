...
void Game::update() {
    ...
    for (auto& brick : bricks) {
        ...
        if (!collisionDetected && ball.getShape().getGlobalBounds().findIntersection(brickBounds)) {
            ...
            // Обновить позицию мяча после удара
    ...
    if (hasRandomTrajectory) {
        if (randomTrajTimer.getElapsedTime().asSeconds() < 2.0f) {
            if (rand() % 200 == 0) {
                ...
                sf::Vector2f newVelocity(currentSpeed * cos(angleRad), currentSpeed * sin(angleRad));
    ...
    // Проверка столкновения с нижней платформой
    if (hasBottomPlatform) {
        auto platformBounds = bottomPlatform.getGlobalBounds();
        if (ball.getShape().getGlobalBounds().findIntersection(platformBounds)) {
            ball.setVelocity({ball.getVelocity().x, -ball.getVelocity().y});
